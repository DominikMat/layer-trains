...
// Ensure STB implementation is only defined once in the project (usually in main or a utility cpp)
// If you get linker errors, remove this define here.
...
public:
    ...
    Texture bake_terrain_texture(bool release_data = true) {
        ...
        // Output path logic
        std::string cleaned_name = terrain_data->title;
        ...
        if (std::ifstream(cache_path).good()) {
            return Texture(cache_path.c_str(), false);
        }
        #endif

        // Load Area Data (Biome Map)
        int width, height, nrChannels;
        unsigned char* map_data = stbi_load(terrain_data->areas_data_path.c_str(), &width, &height, &nrChannels, 3);
        if (!map_data) return ERROR_EMPTY_TEXTURE_RETURN;

        // Load Gradients
        int gw;
        std::vector<vec3> g_elev = load_gradient(GRADIENT_ELEVATION_PATH, gw);
        std::vector<vec3> g_steep = load_gradient(GRADIENT_STEEPNESS_PATH, gw);
        std::vector<vec3> g_water = load_gradient(GRADIENT_WATER_PATH, gw);

        std::vector<unsigned char> color_buffer(width * height * 3);

        auto get_pixel_r = [&](int x, int y) {
            int cx = glm::clamp(x, 0, width-1);
            int cy = glm::clamp(y, 0, height-1);
            return map_data[(cy * width + cx) * 3];
        };
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (y < 2 || x < 2 || x >= width-2 || y >= height-2) {
                    color_buffer[idx] = (unsigned char)(Colour::TERRAIN_SIDE.r * 255);
                    color_buffer[idx+1] = (unsigned char)(Colour::TERRAIN_SIDE.g * 255);
                    color_buffer[idx+2] = (unsigned char)(Colour::TERRAIN_SIDE.b * 255);
                    continue;
                }

                // Calculation
                float h_raw = get_pixel_r(x,y) / 255.0f;
                float elev = terrain_data->minimum_height_reach + h_raw * (terrain_data->maximum_height_reach - terrain_data->minimum_height_reach);
                
                // Steepness
                float h_r = get_pixel_r(x+1, y)/255.f;
                float h_d = get_pixel_r(x, y+1)/255.f;
                float steepness = glm::length(vec2(h_raw - h_r, h_raw - h_d)) * STEEPNESS_SCALE * 5.0f;

                vec3 final_col;

                if (elev < terrain_data->water_level_height) {
                    float d = (terrain_data->water_level_height - elev) / terrain_data->water_level_height;
                    final_col = sample_grad(g_water, d);
                } else {
                    vec3 c_e = sample_grad(g_elev, elev / ELEVATION_GRADIENT_MAX_HEIGHT);
                    vec3 c_s = sample_grad(g_steep, steepness);
                    final_col = mix(c_s, c_e, ELEVATION_GRADIENT_STRENGTH);

                    if (elev > terrain_data->snow_level_height) {
                        float s = (elev - terrain_data->snow_level_height) / SNOW_FALLOFF_RANGE;
                        final_col = mix(final_col, vec3(1.0f), glm::clamp(s, 0.f, 1.f));
                    }
                }

                color_buffer[idx] = (unsigned char)(final_col.r * 255);
                color_buffer[idx+1] = (unsigned char)(final_col.g * 255);
                color_buffer[idx+2] = (unsigned char)(final_col.b * 255);
            }
        }

        if(release_data) stbi_image_free(map_data);
...
private:
    std::vector<vec3> load_gradient(const char* path, int& w) {
        int h, c;
        unsigned char* d = stbi_load(path, &w, &h, &c, 3);
        std::vector<vec3> res;
        if(d) {
            for(int i=0; i<w; i++) res.push_back(vec3(d[i*3]/255.f, d[i*3+1]/255.f, d[i*3+2]/255.f));
            stbi_image_free(d);
        }
        return res;
    }

    vec3 sample_grad(const std::vector<vec3>& g, float t) {
        if(g.empty()) return vec3(1,0,1);
        t = glm::clamp(t, 0.f, 1.f);
        return g[(int)(t * (g.size()-1))];
    }
};
